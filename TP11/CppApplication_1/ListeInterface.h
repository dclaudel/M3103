...
public:
    ...
    virtual bool insereAtPositIter(int nouvellePosition, const TypeInfo& nouvelleInfo) = 0;
    ...
    @param nouvelleInfo  information à insérer dans la liste.
    @return  True si l'insertion est réussie, False sinon. */
    virtual bool insereAtPositRecAvecProc(int nouvellePosition, const TypeInfo& nouvelleInfo) = 0;
     ...
     * la méthode utilise "un WORKER" qui est une PROCEDURE avec un paramètre donnée-résultat 
     ...
     @param position  rang de la cellule visée à supprimer.
     @return  True si la suppression est réussie, False sinon. */
    virtual bool supprimeAtPositRecAvecProc(int position) = 0;
    ...
    /** RECUSIVE rempalcement de l'information portée par la cellule visée au rang position.
     @pre  1 <= position <= getLength().
     @post  l'information de la cellule visée vaut nouvelleInfo.
     @param position  rang de la cellule visée.
     @param nouvelleInfo  nouvelle information de la cellule visée. */
    virtual void setInfoAtPositRec(int position, const TypeInfo& nouvelleInfo) = 0;
}; // end ListInterface
#endif